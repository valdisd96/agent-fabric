...
| **D3** | **Hybrid: state priority first (in-review / in-progress > rework > tests-pending > needs-planning > needs-decompose / unqualified), then `priority:*` across projects, then round-robin between projects at same level, then createdAt** | Drains in-flight; respects priority; no starvation. |
...
Inline reply: replying to a notification message that's tied to an issue posts the reply as a GH comment on that issue, and for Q&A notifications (`clarification`, `decompose-approval`) flips the issue's state label back to its resume state so the next scheduler tick re-dispatches the agent — "/resume magic" promoted from the orchestrator plan's K3 to v1, since Telegram makes it natural. Resume state is `state:needs-decompose` for an epic (issue carries `type:epic`) or for a `decompose-approval` reply; for a triage-stage clarification (qualify-issue, detected via the `<!-- agent-qualify` marker in the embedded comment body) the bot just removes `state:clarification-needed` so the next tick re-qualifies, since at the triage stage there is no plan-exec session to resume; otherwise resume is `state:in-progress` (the plan-exec / clarify-issue contract). The scheduler treats `state:in-progress` as actionable — it routes to `epic-decompose` for `type:epic` issues and to `plan-exec` otherwise — so a non-epic clarification reply (or a dispatch that crashed mid-stream) is picked up on the next tick instead of parking silently. For Q&A transitions, the notification body also embeds the latest `<!-- agent-* -->` comment so the question or proposal is readable directly in the TG message — phone UX, no GH round-trip needed for short answers.
